...
let config = fs.readFileSync(
    path.join(task_server.getDir(), task_server.PATH_CONFIG.FILES.CONFIG),
    task_server.SERVER_CONFIG.CHARSET
);
config = JSON.parse(config);
let config_context = JSON.parse(
    fs.readFileSync(path.join(task_server.getDir(), task_server.PATH_CONFIG.FILES.CONFIG), task_server.SERVER_CONFIG.CHARSET)
); //仅在当前进程中使用，不会写入文件
...
const websocket_port = APP_CONFIG.WEBSOCKET_PORT; //目前只支持8084端口，写死，因为扩展里面写死了
...
if (process.platform === "win32" && process.arch === "ia32") {
    ...
    chromeBinaryPath = path.join(__dirname, APP_CONFIG.PLATFORM_PATHS.WIN32_IA32.CHROME);
    execute_path = path.join(__dirname, APP_CONFIG.PLATFORM_PATHS.WIN32_IA32.EXECUTE);
} else if (process.platform === "win32" && process.arch === "x64") {
    driverPath = path.join(__dirname, APP_CONFIG.PLATFORM_PATHS.WIN32_X64.DRIVER);
    chromeBinaryPath = path.join(__dirname, APP_CONFIG.PLATFORM_PATHS.WIN32_X64.CHROME);
    execute_path = path.join(__dirname, APP_CONFIG.PLATFORM_PATHS.WIN32_X64.EXECUTE);
} else if (process.platform === "darwin") {
    driverPath = path.join(__dirname, APP_CONFIG.PLATFORM_PATHS.DARWIN.DRIVER);
    chromeBinaryPath = path.join(__dirname, APP_CONFIG.PLATFORM_PATHS.DARWIN.CHROME);
    execute_path = path.join(__dirname, APP_CONFIG.PLATFORM_PATHS.DARWIN.EXECUTE);
...
async function beginInvoke(msg, ws) {
    if (msg.type == 1) {
        ...
        if (msg.message.mysql_config_path != TASK_CONSTANTS.SPECIAL_IDS.NEW_TASK.toString()) {
        ...
        ws.send(
            JSON.stringify({
                ...
                reliablespider_location: task_server.getReliableSpiderLocation(),
            })
        );
    } else if (msg.type == 6) {
        try {
...
async function runBrowser(lang = "en", user_data_folder = "", mobile = false) {
    ...
    language = lang;
    if (lang == "en") {
        options.addExtensions(path.join(__dirname, "ReliableSpider_en.crx"));
    } else if (lang == "zh") {
        options.addExtensions(path.join(__dirname, "ReliableSpider_zh.crx"));
    }
